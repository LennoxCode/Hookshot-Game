...
public class RopeAnimationController : MonoBehaviour
{
   ...
   [Header("General Refernces:")]
    public GrapplingController grapplingGun;
    ...
    private void LinePointsToFirePoint()
    {
        ...
        {
            m_lineRenderer.SetPosition(i, grapplingGun.grappleOrigin);
        }
    }
    ...
    void DrawRopeWaves()
    {
        for (int i = 0; i < percision; i++)
        {
            ...
            Vector2 offset = Vector2.Perpendicular(grapplingGun.targetDirection).normalized * ropeAnimationCurve.Evaluate(delta) * waveSize;
            Vector2 targetPosition = Vector2.Lerp(grapplingGun.grappleOrigin, grapplingGun.grapplePoint, delta) + offset;
            Vector2 currentPosition = Vector2.Lerp(grapplingGun.grappleOrigin, targetPosition, ropeProgressionCurve.Evaluate(moveTime) * ropeProgressionSpeed);
    ...
    {
        m_lineRenderer.SetPosition(0, grapplingGun.grappleOrigin);
